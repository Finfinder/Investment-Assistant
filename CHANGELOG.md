...
- Add a `nginx-version-consistency` CI job to `ci.yml` that fails when the default `NGINX_VERSION` in `nginx/Dockerfile` (`ARG NGINX_VERSION=…`) diverges from the new `nginx/.nginxrc` single-source-of-truth file, mirroring the existing frontend Node check; closes [#228](https://github.com/Finfinder/Investment-Assistant/issues/228)
- Add a backend contract test `test_backend_dockerfile_python_version_consistency.py` asserting the default `PYTHON_VERSION` in `backend/Dockerfile` stays in sync with `backend/.python-version` (the interpreter version is a prefix of the image tag, e.g. `3.12` → `3.12-slim`); closes [#228](https://github.com/Finfinder/Investment-Assistant/issues/228)
...
- Extend the Stryker frontend mutation-testing scope in `frontend/stryker.conf.json` to include `src/lib/format.ts` (`confidenceBarClass`, `formatRiskReward`) alongside the existing `src/lib/riskReward.ts`, reusing the existing unit tests and keeping the 70% threshold, raising confidence in the presentation formatting layer ([#214](https://github.com/Finfinder/Investment-Assistant/issues/214))
- Unify the mutation score threshold (70%) into a single source of truth at the repository root (`mutation-threshold.json`), shared by the backend mutmut gate (`backend/scripts/run_mutmut.py` via `load_default_min_score()`) and the frontend Stryker gate (`frontend/stryker.conf.js`); remove the hardcoded `70` from the reusable/scheduled/release workflows and add unit tests asserting the shared threshold on both sides ([#215](https://github.com/Finfinder/Investment-Assistant/issues/215))
- Centralize the Python version in `backend/.python-version` as the single source of truth and read it via `python-version-file` in `reusable-backend.yml`, `reusable-sonarcloud.yml` and `reusable-mutation-testing.yml`, removing the hardcoded `python-version: "3.12"` from those workflows and the `ci.yml`/`release.yml` callers; add a contract test asserting the file satisfies `requires-python` and declare `packaging>=23.0` as an explicit `dev` dependency so the test no longer relies on a transitive install ([#217](https://github.com/Finfinder/Investment-Assistant/issues/217))
- Parameterize the base image tags in `nginx/Dockerfile` (`ARG NGINX_VERSION=1.27-alpine`) and `backend/Dockerfile` (`ARG PYTHON_VERSION=3.12-slim`), sourcing the defaults from the single-source-of-truth files `nginx/.nginxrc` and `backend/.python-version` respectively, following the existing `NODE_VERSION`/`frontend/.nvmrc` pattern; `release.yml` now reads these files and passes `--build-arg`, and `docker-compose.yml` passes them via `build.args` with the same defaults, keeping the `@sha256` digest pins (IA-164) as the authoritative source ([#228](https://github.com/Finfinder/Investment-Assistant/issues/228))

### Fixed

- Fix date-dependent CPI fallback unit tests in `backend/tests/unit/test_cpi_fallback_source.py` by replacing the hardcoded `date(2026, 4, 1)` "fresh" observation periods with a `_fresh_period()` helper (first day of last month, always inside the 90-day freshness window), so the suite no longer fails once the hardcoded date ages past `CPI_MAX_AGE_DAYS`; closes [#265](https://github.com/Finfinder/Investment-Assistant/issues/265)

### Security

- Resolve all 31 open backend dependency vulnerabilities (16 high, 15 moderate) reported by GitHub Dependency Scanning: bump the direct `pydantic-settings` dependency to `>=2.14.2` (GHSA-4xgf-cpjx-pc3j), add explicit lower-bound pins for the patched transitive versions (`starlette>=1.3.1`, `urllib3>=2.7.0`, `idna>=3.15`, `click>=8.3.3`, `mako>=1.3.12`, `msgpack>=1.2.1`, `soupsieve>=2.8.4`) so the resolver is forced above the vulnerable ranges on every re-resolution, and refresh pip to `>=26.1.2` in the `backend/Dockerfile` builder and runtime stages; `pip-audit` now reports 0 vulnerabilities and the obsolete `--ignore-vuln CVE-2026-3219` (pip) flag was removed from the `backend-security` CI job ([#267](https://github.com/Finfinder/Investment-Assistant/issues/267))
...
- Add an `npm audit --audit-level=moderate` gate to the frontend CI by introducing a `frontend-security` job in `reusable-frontend.yml` (installed via the existing `frontend-setup` composite action using `npm ci`); the job blocks PRs that introduce dependencies with `moderate` or higher vulnerabilities, mirroring the backend `pip-audit` gate, and is wired into the `needs` of `frontend-e2e` ([#230](https://github.com/Finfinder/Investment-Assistant/issues/230))
...
- Add mutmut as an optional CI quality gate for `app/modules/technical_analysis/signal_rating.py` with a mutation score threshold of >= 70%; configure `[tool.mutmut]` in `backend/pyproject.toml`, add `backend/scripts/run_mutmut.py` enforcing the threshold, and wire a `mutation-testing` job via `reusable-mutation-testing.yml` into `ci.yml` ([#191](https://github.com/Finfinder/Investment-Assistant/issues/191))
- Extend the mutmut mutation-testing gate (PoC from [#191](https://github.com/Finfinder/Investment-Assistant/issues/191)) to also cover `app/modules/signal_aggregation` (baseline 75.0%) and `app/modules/strategy_generator` (raised from 53.4% to 87.0%) at the same >= 70% threshold; `backend/scripts/run_mutmut.py` now accepts `--modules` (multiple, defaulting to all three) with per-module cache isolation and aggregated pass/fail, `[tool.mutmut]` lists all three scopes, and `reusable-mutation-testing.yml` enforces the threshold independently per module. Genuinely-equivalent mutants (display strings, tuned constants, type annotations, unreachable guards) are marked with `# pragma: no mutate` ([#207](https://github.com/Finfinder/Investment-Assistant/issues/207))
...
- Refactor `app/modules/technical_analysis/indicators.py` to call the consolidated `rate_signal()` from `signal_rating.py` instead of importing 13 named `rate_*` functions; introduce a module-level `_SIGNAL_KEYS` map binding each indicator name to its `rate_signal` config key as the single source of truth, cutting the import block and centralizing the indicator-to-signal mapping ([#190](https://github.com/Finfinder/Investment-Assistant/issues/190))
...
- Promote complete frontend jobs (`frontend-lint`, `frontend-test`, `frontend-e2e`) into a single reusable `reusable-frontend.yml` workflow invoked via `uses:` from `ci.yml` and `release.yml`, eliminating duplication across pipelines ([#187](https://github.com/Finfinder/Investment-Assistant/issues/187))
- Extract backend jobs (`lint`, `architecture-lint`, `type-check`, `test`, `security`) into a single reusable `reusable-backend.yml` workflow invoked via `uses:` from `ci.yml` and `release.yml`, eliminating duplication across pipelines and unifying the `pip-audit` ignored CVE list ([#200](https://github.com/Finfinder/Investment-Assistant/issues/200))
- Relocate `AnalysisPipeline` orchestrator from `app/modules/pipeline.py` to `app/orchestration/pipeline.py` so it no longer violates the import-linter `independence` contract (domain modules must not import each other); add `app/orchestration/__init__.py` and update `app/api/v1/analysis.py` plus affected tests to import from the new path. Add a dedicated `architecture-lint` CI job running `lint-imports` ([#120](https://github.com/Finfinder/Investment-Assistant/issues/120))
- Centralize the frontend Node.js version in a single source of truth `frontend/.nvmrc`, read by the `frontend-setup` composite action via `node-version-file`; remove the hardcoded `"20"` from `frontend-setup`, `reusable-frontend.yml`, `ci.yml` and `release.yml` to prevent version drift across CI, release and SonarCloud pipelines ([#188](https://github.com/Finfinder/Investment-Assistant/issues/188))
