...
Import boundaries are enforced by `import-linter` contracts defined in `pyproject.toml`. The formal domain contract documenting bounded contexts, responsibilities, public APIs and the dependency direction of these modules lives in [`backend/domain.md`](backend/domain.md) — it is the single source of truth for module boundaries and review.
