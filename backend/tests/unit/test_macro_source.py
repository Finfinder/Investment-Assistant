...
class TestMacroSourceRouting:
    @pytest.mark.asyncio
    async def test_fetch_indicator_routes_cpi_jp_to_oecd(
        self,
        mock_fred: MagicMock,
        mock_oecd: MagicMock,
    ...
    @pytest.mark.asyncio
    async def test_fetch_indicator_routes_non_jp_indicators_to_fred(
        self,
        mock_fred: MagicMock,
        mock_oecd: MagicMock,
        mock_cpi_fallback: MagicMock,
    ...
    @pytest.mark.asyncio
    async def test_fetch_indicator_routes_cpi_us_to_cpi_fallback(
        self,
        mock_fred: MagicMock,
        mock_oecd: MagicMock,
        mock_cpi_fallback: MagicMock,
    ):
        mock_cpi_fallback.fetch_indicator.return_value = 2.4
        source = MacroDataSource(fred=mock_fred, oecd=mock_oecd, cpi_fallback=mock_cpi_fallback)

        result = await source.fetch_indicator("cpi_us")

        assert result == pytest.approx(2.4)
        mock_cpi_fallback.fetch_indicator.assert_awaited_once_with("cpi_us", 365)
        mock_fred.fetch_indicator.assert_not_awaited()
        mock_oecd.fetch_jp_cpi_yoy.assert_not_awaited()
    ...
    @pytest.mark.asyncio
    async def test_fetch_multiple_routes_mixed_indicator_list(
        self,
        mock_fred: MagicMock,
        mock_oecd: MagicMock,
        mock_cpi_fallback: MagicMock,
        ...
        mock_cpi_fallback.fetch_indicator.return_value = 2.0
        ...
        result = await source.fetch_multiple(["cpi_jp", "cpi_ca", "fed_funds_rate", "boj_rate"])
        ...
        mock_cpi_fallback.fetch_indicator.assert_awaited_once_with("cpi_ca", 365)
